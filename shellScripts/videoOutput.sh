...
IMAGE_DURATION=$2
IMAGE_DURATION=`python -c "print(round($IMAGE_DURATION))"`
...
FILES=`find $3/*`                         # USE ALL IMAGES UNDER THE media FOLDER
...
# 9. END
FULL_SCRIPT+=" -map [video] -vsync 2 -async 1 -rc-lookahead 0 -g 0 -profile:v main -level 42 -c:v libx264 -r ${FPS} ${SAVE_PATH}/${1}.mp4"
